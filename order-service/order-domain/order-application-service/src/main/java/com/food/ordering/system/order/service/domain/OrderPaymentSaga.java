...
public class OrderPaymentSaga implements SagaStep<PaymentResponse> {
    ...
    public void process(PaymentResponse paymentResponse) {
        ...
        SagaStatus sagaStatus = orderSagaHelper.orderStatusToSagaStatus(orderPaidEvent.order().getOrderStatus());
        paymentOutboxHelper.save(getUpdatedPaymentOutboxMessage(orderPaymentOutboxMessage,
                orderPaidEvent.order().getOrderStatus(), sagaStatus));

        approvalOutboxHelper.saveApprovalOutboxMessage(orderDataMapper.orderPaidEventToOrderApprovalEventPayload(orderPaidEvent),
                orderPaidEvent.order().getOrderStatus(),
                sagaStatus,
                OutboxStatus.STARTED,
                UUID.fromString(paymentResponse.getSagaId()));

        log.info("Order with id: {} is paid", orderPaidEvent.order().getId().value());
    }
    ...
    private OrderApprovalOutboxMessage getUpdatedApprovalOutboxMessage(String sagaId,
        ...
        orderApprovalOutboxMessage.setProcessedAt(ZonedDateTime.now(DomainConstants.UTC));
    ...
    private OrderPaymentOutboxMessage getUpdatedPaymentOutboxMessage(OrderPaymentOutboxMessage orderPaymentOutboxMessage,
                                                                     OrderStatus orderStatus,
                                                                     SagaStatus sagaStatus) {
        orderPaymentOutboxMessage.setProcessedAt(ZonedDateTime.now(DomainConstants.UTC));
