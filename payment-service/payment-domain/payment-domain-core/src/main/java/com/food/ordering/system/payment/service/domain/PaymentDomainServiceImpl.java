...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCompletedEvent(payment, ZonedDateTime.now(UTC), paymentCompletedEventDomainEventPublisher);
        } else {
            var errorMsg = "Payment initiation is failed for order id: %s"
                    .formatted(payment.getOrderId().value());
            log.info(errorMsg);
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment, ZonedDateTime.now(UTC), failureMessages, paymentFailedEventDomainEventPublisher);
        }
    }
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        ...
        if (totalDebitHistory.isGreaterThan(totalCreditHistory)) {
            var errorMsg = "Customer with id: %s doesn't have enough credit according to credit history."
        ...
        if (!creditEntry.getTotalCreditAmount().equals(totalCreditHistory.subtract(totalDebitHistory))) {
            var errorMsg = "Credit history total is not equal to current credit for customer id: %s."
                    .formatted(creditEntry.getCustomerId().value());
    ...
    private Money getTotalHistoryAmount(List<CreditHistory> creditHistories, TransactionType transactionType) {
        return creditHistories.stream()
                .filter(it -> transactionType.equals(it.getTransactionType()))
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())) {
            var errorMsg = "Customer with id: %s doesn't have enough credit for payment."
                    .formatted(payment.getCustomerId().value());
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(UTC), paymentCancelledEventDomainEventPublisher);
        } else {
