...
public class PaymentRequestKafkaListener implements KafkaConsumer<PaymentRequestAvroModel> {
    ...
    public void receive(@Payload List<PaymentRequestAvroModel> messages,
        ...
        messages.forEach(it -> {
            try {
                switch (it.getPaymentOrderStatus()) {
                    case PENDING -> {
                        log.info("Processing payment for order id: {}", it.getOrderId());
                        var paymentRequest = paymentMessagingDataMapper.paymentRequestAvroModelToPaymentRequest(it);
                        paymentRequestMessageListener.completePayment(paymentRequest);
                    }
                    case CANCELLED -> {
                        log.info("Cancelling payment for order id: {}", it.getOrderId());
                        var paymentRequest = paymentMessagingDataMapper.paymentRequestAvroModelToPaymentRequest(it);
                ...
                if (sqlException != null && sqlException.getSQLState() != null &&
                    ...
                    log.error("Caught unique constraint exception with sql state: {} " +
                            ...
                            sqlException.getSQLState(), it.getOrderId());
                } else {
                ...
                log.error("No payment found for order id: {}", it.getOrderId());
            }
